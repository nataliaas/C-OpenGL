...
const GLdouble zNEAR = 50.0;
const GLdouble zFAR = 70.0;
...
void Reshape(int width, int height)
{
    ...
    if (aspect == ASPECT_1_1)
    {
        ...
        if (width < height && width > 0)
            glFrustum(left, right, bottom * height / width, top * height / width, zNEAR, zFAR);
        else

            // szerokoœæ okna wiêksza lub równa wysokoœci okna
            if (width >= height && height > 0)
                glFrustum(left * width / height, right * width / height, bottom, top, zNEAR, zFAR);
